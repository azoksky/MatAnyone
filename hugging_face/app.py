...
def parse_augment():
    ...
    parser.add_argument("--hard-mask", action="store_true", help="Use a binary (hard) alpha mask with sharp edges.")
                ...
                with gr.Row():
                    with gr.Accordion('MatAnyone Settings (click to expand)', open=False):
                        ...
                        with gr.Row():
                         ...
                         invert_alpha_checkbox = gr.Checkbox(label="Invert Alpha Mask",value=True,info="Output alpha as background=white (inverted).",interactive=True)
                        with gr.Row():
